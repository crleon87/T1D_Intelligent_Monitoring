...
patients_best = pd.read_csv('../Best_and_worst/best_unique_list.csv')
patients_worst = pd.read_csv('../Best_and_worst/worst_unique_list.csv')
patients = patients_best['0'].values.tolist() + patients_worst['0'].values.tolist()

for current_patient in patients:

    print('-------------------------------------------------------------------------------------------------------')
    print(F'------------- START PATIENT {current_patient} ---------------------------------------------------------')
    print('-------------------------------------------------------------------------------------------------------')
    ...
    log_file = open(f"script_log_cluster_{current_patient}.txt", "a")
    ...
    def read_windows_no_duplicated(horizon, patient):
        file_x_train = f"/Personalized_patients_FULL/FULL/x_train_windows_horizon_{horizon}_{patient}.npy"
        file_y_train = f"/Personalized_patients_FULL/FULL/y_train_windows_horizon_{horizon}_{patient}.npy"

        file_x_val = f"/Personalized_patients_FULL/FULL/x_val_windows_horizon_{horizon}_{patient}.npy"
        file_y_val = f"/Personalized_patients_FULL/FULL/y_val_windows_horizon_{horizon}_{patient}.npy"

        file_x_test = f"/Personalized_patients_FULL/FULL/x_test_windows_horizon_{horizon}_{patient}.npy"
        file_y_test = f"/Personalized_patients_FULL/FULL/y_test_windows_horizon_{horizon}_{patient}.npy"
    ...
    ## LSTM, CNN, and linear model architectures ################################################
    ...
    class LSTMModel:
        ...
        def __repr__(self):
            return 'LSTM_{0}_units_{1}_layers_dropout={2}_{3}'.format(self.nb_hidden_units, self.nb_layers,
                                                                      self.dropout,
    ...
    def clarke_error_grid(ref_values, pred_values, title_string, show_plot=False):
        ...
        if show_plot:
            ...
            plt.savefig(f'plots/Clarke_Error_Grid_{title_string}.jpg', dpi=350, bbox_inches='tight')
        ...
        for i in range(6):
            ...
            if reference_values.shape[0] != 0:
                zone = clarke_error_grid(ref_values=reference_values.values, pred_values=pred_values.values,
                                         title_string=f'patient{current_patient}_{grid_name}{list_dataframe_by_range_name[i]}',
                                         show_plot=True)
                ...
                print(f'No values in range {list_dataframe_by_range_name[i]}')
                print()

                new_row = {'Model': list_dataframe_by_range_name[i], 'A': float('NaN'), 'B': float('NaN'),
                           'C': float('NaN'), 'D': float('NaN'), 'E': float('NaN'),
                           'A + B': float('NaN'), 'RMSE': float('NaN'), 'MSE': float('NaN'), 'MAE': float('NaN'),
                           'MAPE': float('NaN')}
                new_index = len(df_metrics_summary)
                df_metrics_summary.loc[new_index] = new_row
    ...
    # def get_patient_list_from_dict(current_dict: dict):
    ...
    for current_horizon in horizons:  # For all horizons -------------------------------------
        ...
        for current_model in models:  # For all models ---------------------------------------
            ...
            print("######################################################################## - ")
            print(f'START training cluster{current_patient}_{current_model} horizon={current_horizon}')
            print()
            ...
            hist, model_trained = train(x_train=x_train, y_train=y_train, x_val=x_val, y_val=y_val, model=model,
                                        ...
                                        save_filepath=f"models/patient_{current_patient}_{current_model}")
            ...
            get_train_plots_loss(hist, f"plots/patient_{current_patient}_{current_model}_H{current_horizon}")
            get_train_plots_MSE(hist, f"plots/patient_{current_patient}_{current_model}_H{current_horizon}")

            df_hist = pd.DataFrame(data=hist.history)
            df_hist.to_csv(f'hist/df_hist_patient_{current_patient}_{current_model}_H{current_horizon}.csv')

            hist_best = df_hist.iloc[df_hist['loss'].idxmin()]
            hist_best['epoch'] = df_hist.shape[0]
            hist_best.to_csv(f'hist/hist_best_patient{current_patient}_{current_model}_H{current_horizon}.csv')

            print()
            print(f'END training patient_{current_patient}_{current_model} horizon={current_horizon}')
            print("########################################################################")
            print()
            ...
            print(
                f'Test results for patient_{current_patient}_{current_model} horizon={current_horizon} participant {current_patient} - START')
            ...
            if current_model == 'Linear':  # load linear model
                model_load = prepare_model_linear(history_length,
                                                  weights=f"models/patient_{current_patient}_{current_model}{current_horizon}")
            elif current_model == 'LSTM':  # load LSTM model
                model_load = prepare_model_LSTM(history_length,
                                                weights=f"models/patient_{current_patient}_{current_model}{current_horizon}")
            else:  # load CNN model
                model_load = prepare_model_CNN(history_length,
                                               weights=f"models/patient_{current_patient}_{current_model}{current_horizon}")
            ...
            elapsed_time = end_time - start_time
            print(
                f"Testing time for patient_{current_patient}_{current_model} horizon={current_horizon}: {elapsed_time} seconds")

            # Save predictions and references
            print('Save predictions and references')
            df_test_results = pd.DataFrame({'y_test': y_test.ravel(), 'y_predict': bg_predict.ravel()})
            df_test_results.to_parquet(
                f'test_results/df_test_results_vectors_patient_{current_patient}_{current_model}_H{current_horizon}.parquet')

            print('Save results by range')
            df_full_range_metrics = test_by_range(df_test_results, f'{current_model}_H{current_horizon}_')
            df_full_range_metrics.to_csv(
                f'test_results/results_by_range_patient_{current_patient}_{current_model}_H{current_horizon}.csv',
                index=False)

            print(
                f'Test results of {current_model} horizon={current_horizon} patient={current_patient} patient={current_patient} - DONE')
    ...
    print('-------------------------------------------------------------------------------------------------------')
    print(F'------------- END PATIENT {current_patient} ---------------------------------------------------------')
    print('-------------------------------------------------------------------------------------------------------')
