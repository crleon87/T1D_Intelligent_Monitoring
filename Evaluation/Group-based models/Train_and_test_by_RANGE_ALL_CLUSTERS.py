...
patience = 50
max_epoch = 500
parcial_windows_path = '/Cluster windows_FULL_Horizons/'
...
for current_cluster in clusters_list:
    windows_path = f'{parcial_windows_path}cluster{current_cluster}/'

    print('-------------------------------------------------------------------------------------------------------')
    print(F'------------- START CLUSTER {current_cluster} ---------------------------------------------------------')
    print('-------------------------------------------------------------------------------------------------------')
    ...
    def test_by_range(df_result_vector: pd.DataFrame, grid_name:str)->pd.DataFrame:
        ...
        for i in range(6):
            ...
            zone = clarke_error_grid(ref_values=reference_values.values, pred_values=pred_values.values,
                                     title_string=f'cluster{current_cluster}_{grid_name}{list_dataframe_by_range_name[i]}', show_plot=True)
    ...
    for current_horizon in horizons:  # For all horizons -------------------------------------
        ...
        for current_model in models:  # For all models ---------------------------------------
            ...
            print("######################################################################## - ")
            print(f'START training cluster{current_cluster}_{current_model} horizon={current_horizon}')
            print()
            ...
            hist, model_trained = train(x_train=x_train, y_train=y_train, x_val=x_val, y_val=y_val, model=model,
                                        horizon=current_horizon, save_filepath=f"models/cluster{current_cluster}_{current_model}")
            ...
            get_train_plots_loss(hist, f"plots/cluster{current_cluster}_{current_model}_H{current_horizon}")
            get_train_plots_MSE(hist, f"plots/cluster{current_cluster}_{current_model}_H{current_horizon}")

            df_hist = pd.DataFrame(data=hist.history)
            df_hist.to_csv(f'hist/df_hist_cluster{current_cluster}_{current_model}_H{current_horizon}.csv')

            hist_best = df_hist.iloc[df_hist['loss'].idxmin()]
            hist_best['epoch'] = df_hist.shape[0]
            hist_best.to_csv(f'hist/hist_best_cluster{current_cluster}_{current_model}_H{current_horizon}.csv')

            print()
            print(f'END training cluster{current_cluster}_{current_model} horizon={current_horizon}')
            print("########################################################################")
            print()

            # ## TEST ################################

            print(f'Test results for cluster{current_cluster}_{current_model} horizon={current_horizon}')
            ...
            if current_model == 'Linear':  # load linear model
                model_load = prepare_model_linear(history_length,
                                                  weights=f"models/cluster{current_cluster}_{current_model}{current_horizon}")  # Linear4
            else:
                model_load = prepare_model_LSTM(history_length, weights=f"models/cluster{current_cluster}_{current_model}{current_horizon}")
            ...
            elapsed_time = end_time - start_time
            print(f"Testing time for cluster{current_cluster}_{current_model} horizon={current_horizon}: {elapsed_time} seconds")

            # Save predictions and references
            df_test_results = pd.DataFrame({'y_test': y_test.ravel(), 'y_predict': bg_predict.ravel()})
            df_test_results.to_parquet(f'test_results/df_test_results_vectors_cluster{current_cluster}_{current_model}_H{current_horizon}.parquet')

            df_full_range_metrics = test_by_range(df_test_results, f'{current_model}_H{current_horizon}_')
            df_full_range_metrics.to_csv(f'test_results/results_by_range_cluster{current_cluster}_{current_model}_H{current_horizon}.csv')
    ...
    print('-------------------------------------------------------------------------------------------------------')
    print(F'------------- END CLUSTER {current_cluster} ---------------------------------------------------------')
    print('-------------------------------------------------------------------------------------------------------')